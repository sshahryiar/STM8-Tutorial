...
1244                     ; 311 }
1247  014d 81            	ret
1287                     ; 320 uint16_t ADC2_GetConversionValue(void)
1287                     ; 321 {
1288                     	switch	.text
1289  014e               _ADC2_GetConversionValue:
1291  014e 5205          	subw	sp,#5
1292       00000005      OFST:	set	5
1295                     ; 322   uint16_t temph = 0;
1297                     ; 323   uint8_t templ = 0;
1299                     ; 325   if ((ADC2->CR2 & ADC2_CR2_ALIGN) != 0) /* Right alignment */
1301  0150 c65402        	ld	a,21506
1302  0153 a508          	bcp	a,#8
1303  0155 2715          	jreq	L725
1304                     ; 328     templ = ADC2->DRL;
1306  0157 c65405        	ld	a,21509
1307  015a 6b03          	ld	(OFST-2,sp),a
1309                     ; 330     temph = ADC2->DRH;
1311  015c c65404        	ld	a,21508
1312  015f 5f            	clrw	x
1313  0160 97            	ld	xl,a
1314  0161 1f04          	ldw	(OFST-1,sp),x
1316                     ; 332     temph = (uint16_t)(templ | (uint16_t)(temph << (uint8_t)8));
1318  0163 1e04          	ldw	x,(OFST-1,sp)
1319  0165 7b03          	ld	a,(OFST-2,sp)
1320  0167 02            	rlwa	x,a
1321  0168 1f04          	ldw	(OFST-1,sp),x
1324  016a 2021          	jra	L135
1325  016c               L725:
1326                     ; 337     temph = ADC2->DRH;
1328  016c c65404        	ld	a,21508
1329  016f 5f            	clrw	x
1330  0170 97            	ld	xl,a
1331  0171 1f04          	ldw	(OFST-1,sp),x
1333                     ; 339     templ = ADC2->DRL;
1335  0173 c65405        	ld	a,21509
1336  0176 6b03          	ld	(OFST-2,sp),a
1338                     ; 341     temph = (uint16_t)((uint16_t)((uint16_t)templ << 6) | (uint16_t)((uint16_t)temph << 8));
1340  0178 1e04          	ldw	x,(OFST-1,sp)
1341  017a 4f            	clr	a
1342  017b 02            	rlwa	x,a
1343  017c 1f01          	ldw	(OFST-4,sp),x
1345  017e 7b03          	ld	a,(OFST-2,sp)
1346  0180 97            	ld	xl,a
1347  0181 a640          	ld	a,#64
1348  0183 42            	mul	x,a
1349  0184 01            	rrwa	x,a
1350  0185 1a02          	or	a,(OFST-3,sp)
1351  0187 01            	rrwa	x,a
1352  0188 1a01          	or	a,(OFST-4,sp)
1353  018a 01            	rrwa	x,a
1354  018b 1f04          	ldw	(OFST-1,sp),x
1356  018d               L135:
1357                     ; 344   return ((uint16_t)temph);
1359  018d 1e04          	ldw	x,(OFST-1,sp)
1362  018f 5b05          	addw	sp,#5
1363  0191 81            	ret
1407                     ; 352 FlagStatus ADC2_GetFlagStatus(void)
1407                     ; 353 {
1408                     	switch	.text
1409  0192               _ADC2_GetFlagStatus:
1413                     ; 355   return (FlagStatus)(ADC2->CSR & ADC2_CSR_EOC);
1415  0192 c65400        	ld	a,21504
1416  0195 a480          	and	a,#128
1419  0197 81            	ret
1442                     ; 363 void ADC2_ClearFlag(void)
1442                     ; 364 {
1443                     	switch	.text
1444  0198               _ADC2_ClearFlag:
1448                     ; 365   ADC2->CSR &= (uint8_t)(~ADC2_CSR_EOC);
1450  0198 721f5400      	bres	21504,#7
1451                     ; 366 }
1454  019c 81            	ret
1478                     ; 374 ITStatus ADC2_GetITStatus(void)
1478                     ; 375 {
1479                     	switch	.text
1480  019d               _ADC2_GetITStatus:
1484                     ; 376   return (ITStatus)(ADC2->CSR & ADC2_CSR_EOC);
1486  019d c65400        	ld	a,21504
1487  01a0 a480          	and	a,#128
1490  01a2 81            	ret
1514                     ; 384 void ADC2_ClearITPendingBit(void)
1514                     ; 385 {
1515                     	switch	.text
1516  01a3               _ADC2_ClearITPendingBit:
1520                     ; 386   ADC2->CSR &= (uint8_t)(~ADC2_CSR_EOC);
1522  01a3 721f5400      	bres	21504,#7
1523                     ; 387 }
1526  01a7 81            	ret
1539                     	xdef	_ADC2_ClearITPendingBit
1540                     	xdef	_ADC2_GetITStatus
1541                     	xdef	_ADC2_ClearFlag
1542                     	xdef	_ADC2_GetFlagStatus
1543                     	xdef	_ADC2_GetConversionValue
1544                     	xdef	_ADC2_StartConversion
1545                     	xdef	_ADC2_ExternalTriggerConfig
1546                     	xdef	_ADC2_ConversionConfig
1547                     	xdef	_ADC2_SchmittTriggerConfig
1548                     	xdef	_ADC2_PrescalerConfig
1549                     	xdef	_ADC2_ITConfig
1550                     	xdef	_ADC2_Cmd
1551                     	xdef	_ADC2_Init
1552                     	xdef	_ADC2_DeInit
1571                     	end
