...
void main(void)
{
	...
	OLED_print_string(0, 0, "CAN Node 1");
	OLED_print_string(0, 1, "Node 1 ID       1174");
	OLED_print_string(0, 2, "DS18B20 T/ 'C: ");
	OLED_print_string(0, 3, "DHT11  RH/% :");
	OLED_print_string(0, 4, "DHT11     T/'C :");
	OLED_print_string(0, 5, "RX Node ID");
	OLED_print_string(0, 6, "RX Data");
	
	while(1)
	{
		tx_buffer[0] = DS18B20_get_temperature();
		...
		OLED_print_chr(103, 2, tx_buffer[0] );
		OLED_print_chr(103, 3, rx_buffer[0]);
		OLED_print_chr(103, 4, rx_buffer[1]);
...
void setup_CAN(void)
{
	...
	CAN_FilterInit(CAN_FilterNumber_0,
							 ...
							 0x08,
							 0x24,
							 0x68);			//Check messages from ID: 0x1234  // Check Filter Register Table
...
void send_data_to_CAN_bus(void)
{
    ...
    CAN_Transmit(0x0496,  CAN_Id_Extended, CAN_RTR_Data, 2, tx_buffer);
